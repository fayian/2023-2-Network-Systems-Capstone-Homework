...
class Setting:
    ...
    def __init__(
        ...
        if max_collision_wait_time is None:
            self.max_collision_wait_time = coefficient * host_num * self.packet_time  # TODO: this is not complete, change this (hint: add two parameters)
        else:
            self.max_collision_wait_time = max_collision_wait_time

        if p_resend is None:
            self.p_resend = 1 / (coefficient * host_num)  # TODO: this is not complete, change this (hint: add one parameters)
        else:
