...
def update_status(setting, hosts, history, slotted_aloha: bool):
    for h in hosts:
        if h["status"] == 1:
...
def print_history(setting, hosts):
        ...
        for h in hosts:
            ...
            if h["status"] == 0:
                if h["wait_time"] > 0:
                    ...
                    if protocol == "aloha":        # TODO:  change "xxx" to the correct protocol name
                        h["action_to_do"] = 1
                        h["remain_length"] = setting.packet_time
                    
                    elif protocol == "slotted_aloha" and t % setting.packet_time == 0:  # TODO:  change "xxx" to the correct protocol name
                        h["action_to_do"] = 1       
                        h["remain_length"] = setting.packet_time
                    
                    elif protocol == "csma" or protocol == "csma_cd":  # TODO:  change "xxx" to the correct protocol name
            ...
            elif h["status"] == 1 and protocol == "csma_cd":    # TODO:  change "xxx" to the correct protocol name
                others_sending = False
                for others in hosts:
                    if others["id"] == h["id"]:
                        continue
            ...
            elif h["status"] == 2 and protocol == "slotted_aloha" and t % setting.packet_time == 0: # TODO:  change "xxx" to the correct protocol name
                r = random.random()
...
def aloha(setting, show_history=False):
    ...
    for t in range(setting.total_time):
        ...
        ###  Update the host's status and history
        update_status(setting, hosts, history, slotted_aloha=False)

    ### Print the history of each host
    if show_history:
        print_history(setting, hosts)
...
def slotted_aloha(setting, show_history=False):
    ...
    for t in range(setting.total_time):
        ...
        ###  Update the host's status and history
        update_status(setting, hosts, history, slotted_aloha=True)
...
def csma(setting, one_persistent=False, show_history=False):
    # return simulation(setting, "csma", one_persistent=one_persistent, show_history=show_history)
    ### init
    hosts = hosts_template(setting)
    packets_times = setting.gen_packets() # Generate packets for each host
    total_idle_time = 0
...
def csma_cd(setting, one_persistent=False, show_history=False):
    # return simulation(setting, "csma_cd", one_persistent=one_persistent, show_history=show_history)
    ### init
    hosts = hosts_template(setting)
    packets_times = setting.gen_packets() # Generate packets for each host
    total_idle_time = 0
